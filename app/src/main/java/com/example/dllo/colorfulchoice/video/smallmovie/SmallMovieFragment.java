package com.example.dllo.colorfulchoice.video.smallmovie;
...
public class SmallMovieFragment extends BaseFragment implements XListView.IXListViewListener{
    private SmallMovieAdapter smallMovieAdapter;
        ...
        try {
            netTool.post(url, cookie, VideoBean.class, new NetTool.NetListener<VideoBean>() {
                ...
                public void onSuccess(VideoBean videoBean) {
                    ...
                    smallMovieAdapter = new SmallMovieAdapter(resultBeanList, getContext(), R.layout.fragment_video_child_listview);
                    Log.d("Exercise", "resultBeanList.size():" + resultBeanList.size());
                    xListView.setAdapter(smallMovieAdapter);
                    smallMovieAdapter.notifyDataSetChanged();
    ...
    public void onLoadMore() {
        String url = URLValue.VIDEO_SMALLMOVIE_FIRST_URL + informationQuantity + URLValue.VIDEO_SMALLMOVIE_SECOND_URL + (informationQuantity += 5) + URLValue.VIDEO_SMALLMOVIE_THIRD_URL;
        getBean(url, URLValue.VIDEO_COOKIE);
    }

    public class SmallMovieAdapter extends CommonAdapter<VideoBean.ResultBean> {
        private Handler mHandler;
        private Handler handler;
        private Context context;
        private CommonViewHolder videoViewHolder = null;

        public SmallMovieAdapter(List<VideoBean.ResultBean> beanList, Context context, int id) {
